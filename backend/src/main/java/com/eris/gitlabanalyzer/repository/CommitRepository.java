...
public interface CommitRepository extends JpaRepository<Commit,Long> {
    ...
    @Query("select c from Commit c where c.project.id = ?1 and c.mergedAt >= ?2 and c.mergedAt <= ?3 order by c.createdAt asc")
    List<Commit> findAllWithMergeRequestByProjectIdAndDateRange(Long projectId, OffsetDateTime startDateTime, OffsetDateTime endDateTime);
    ...
    @Query("select c from Commit c where c.project.id=?1 and c.mergedAt >= ?3 and c.mergedAt <= ?4 and c.authorEmail in (select ca.authorEmail from CommitAuthor ca where ca.authorEmail = c.authorEmail and ca.gitManagementUser.id = ?2)")
    List<Commit> findByProjectIdAndGitManagementUserIdAndDateRange(Long projectId, Long gitManagementUserId, OffsetDateTime startDateTime, OffsetDateTime endDateTime);
